...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const { paymentId, qrCodeBase64, qrCodeTexto } = await criarPagamentoPix({
      revendedorId: revId,
      email: revendedor.email,
      valor,
      descricao: 'Mensalidade do painel de revenda',
    });

    if (!qrCodeBase64 || !qrCodeTexto) {
      return res.status(500).json({ erro: 'O Mercado Pago não retornou o PIX corretamente.' });
    }

    return res.status(200).json({ ok: true, paymentId, qrCodeBase64, qrCodeTexto });
  } catch (err) {
    console.error('Erro em /api/gerar-pix-mensalidade:', err.detalhes || err);
    return res.status(500).json({ erro: 'Erro ao gerar o PIX no Mercado Pago.' });
  }
};
